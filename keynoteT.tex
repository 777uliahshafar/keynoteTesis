...
\only<1>{Kawasan waterfront memiliki {\large karakteristik} dan {\large perhatian} khusus mengingat pentingnya air sebagai sumber kehidupan.}

\onslide<2->{Berdasarkan rencana tata ruang wilayah kota, Parepare menetapkan kawasan strategis kota yang mendasari pembangunan infrastruktur. Menurut Amanda(2020), pengembangan kawasan objek wisata masih menjadi prioritas Pemerintah.  }

\notec{Indonesia menjadi negara yang memiliki daerah-daerah dengan garis pantai terpanjang. Salah satunya adalah kota Parepare.}
...
\vspace{4pt}
\only<2>{\centering Penampakan Tepi laut dulu}
...
\end{columns}
\vspace{4pt}
\onslide<3->{\centering Penampakan Tepi laut sekarang}
}
\notec{ \textit{(next before read)}
Pengembangan ini menciptkaan dua ruang yang berbeda. Menurut Kim (2012), ruang merupakan tempat yang dapat meningkatkan kualitas hidup dengan  memenuhi kebutuhan individu.

Pemenuhan kebutuhan individu dapat tercapai melalui pemahaman preferensi terhadap ruang yang lebih baik.}

\end{frame}}%end cca

\begin{frame}[label=tesis]{Rumusan Masalah}
\onslide<1->{Menurut Hussein(2014), {\Large pengembangan tepi laut} yang berhasil mampu menarik masyarakat untuk {\large datang ke pesisir.}}
...
\notec{Pengembangan yang terjadi pada tepi laut Senggol menyisahkan dua ruang yang berbeda.
}
...
%        \caption{Step response of a 1st order system.}
    ...
    \end{column}
\end{columns}
\notec{Perbedaan ini dapat menimbulkan orang memilih ruang satu daripada lainnya. Sehingga ruang yang jarang terpilih akan kehilangan sejumlah pengunjung sedangkan yang satunya memiliki banyak pengunjung. Secara keseluruhan, penampakan ini akan memunculkan dua kondisi yang kontras dalam satu kawasan. (next)

Agar kedua ruang tersebut memiliki potensi yang sama untuk dipilih atau menarik pengunjung yang sama, maka mengetahui kebutuhan masyarakat terhadap ruang adalah sangat penting.
...
Menurut Mudureira (2018), pemahaman terkait {\Large preferensi orang terhadap ruang} akan menjelaskan {\large kebutuhan masyarakat}
...
\end{enumerate}
\notec{
(read before slide) Berdasarkan permasalahan itu, penelitian ini menyelidiki preferensi  masyarakat terhadap ruang di kawasan tepi laut senggol. Maka penelitian ini menjawab sejumlah pertanyaan penelitian sebagai berikut:}
\end{frame}}
%------------------------------------------------------------------------------------
...
{\cca
\begin{frame}[label=tin]{Variabel Penelitian}
\onslide<1->{Aspek Ruang:\\\textbf{Aksesibilitas}: Kemampuan individu untuk mendekati sesuatu (La Rosa et al. , 2018).
}

\onslide<2->{\textbf{Keamanan}: Bentuk perlindungan pada diri sendiri, keluarga, dan teman (Carr et al., 1992).
}

\onslide<3->{\textbf{Estetika}: Kebutuhan akan rasa keindahan (Hradilova et al., 2013).

}

\onslide<4->{\textbf{Fasilitas}: Penggunaan untuk memfasilitas dan mendukung aktivitas (Carmona et al., 2003).
}
...
\onslide<1->{Elemen Ruang:\\ \textbf{Jumlah pohon}}
...
\end{minipage}}
\end{center}
\begin{textblock*}{2cm}(10cm,6cm) % {block width} (coords)
...
\section{Pembahasan}
\begin{frame}[label=obj]{Gambaran responden} \vspace{2pt}
...
{\cca
\begin{frame}[t,label=hsl]{Ruang yang paling disukai} \vspace{4pt}
\begin{center}
    \includegraphics[width=.65\textwidth,trim= 1cm .3cm .5cm .6cm,clip]{figures/pieRuang}\\
\end{center}
\end{frame}}

\begin{frame}[t,label=hsl]{Keragaman preferensi terhadap ruang}
\centering
...
{\cca
\begin{frame}[label=hsl2]{Preferensi terhadap aspek} \vspace{4pt}
%    \small
\scalebox{0.85}{
...
Total  & 47 (31\%) & 7 (5\%)  & 46 (31\%) & 50 (33\%) &150 (100\%) \\
 ...
 \bottomrule
\end{tabularx}}
\notec{Ruang ini memberi kesan lapang tapi tetap memiliki estetika sehingga membuat kita menjadi nyaman ketika berada di ruang tersebut.
...
\begin{frame}[label=hsl2]{Keragaman preferensi terhadap aspek}
\begin{figure}[htpb]
    \centering
...
\begin{withoutheadline}
\ccg{
\begin{frame}[label=hsl2]{Keragaman aspek diantara gender}
\begin{minipage}{.45\textwidth}
...
\begin{withoutheadline}
\ccg{
\begin{frame}[label=hsl2]{Keragaman aspek diantara Usia}
\begin{minipage}{.40\textwidth}
...
\begin{withoutheadline}
\ccg{
\begin{frame}[label=hsl2]{Keragaman aspek diantara Suku}
\begin{minipage}{.45\textwidth}
...
\begin{withoutheadline}
\ccg{
\begin{frame}[label=hsl2]{Keragaman aspek diantara Pendidikan}
\begin{minipage}{.40\textwidth}
...
Cukup rindang  &2 (2\%)  & \ccta{28 (33\%)}    &30(14\%) \\
Sangat rindang  &4 (5\%)  & 6 (7\%)    &10(4) \\

1-3m  &3 (4\%)  & 6 (7\%)   &9(4\%) \\
> 3m  &\ccta{35 (41\%)}  & 7 (8\%)    &42(19\%)\\
...
pencahayaan cukup   &5 (6\%)  & \ccta{31 (36\%)}    &36(16\%) \\
...
{\cca
\begin{frame}[label=hsl2]{Preferensi terhadap kelompok elemen}
...
\begin{frame}[label=hsl2]{Keragaman preferensi terhadap klp elemen}
\begin{figure}[htpb]
    \centering
...
\begin{itemize}
    ...
    \item<3-> Elemen yang paling disukai oleh pengunjung adalah lebar jalan dalam memilih ruang tertentu. Pernyataan ini mendukung aspek aksesibilitas yang merupakan aspek yang paling disukai kedua.
\end{itemize}
