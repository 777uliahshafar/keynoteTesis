...
\title{Preferensi Pengunjung terhadap Ruang Publik di Kawasan Tepi Laut Senggol Parepare}
\author[Shafar]{Pembimbing:\\Dr. Ars. Ir. Wijayanti, M.Eng\\Prof. Dr. Ir. Atik Suprapti, M.T. }
...
{\setbeamercolor{background canvas}{bg=digiPH_undip}
...
\begin{frame}[label=title]
	...
	% \maketitle funktioniert auch im Article-Modus
	 \titlepage
     \large{Muhammad Uliah Shafar}
\rule{\textwidth}{.4pt}
\includegraphics[width=2cm]{logos/logo2.png}

\notec{Assalamualaikum wr. wb, salam sejahtera bagi kita semua, kepada dewan penguji yang saya hormati perkenalkan nama sya mh. uliah shfr, pada kesempatan kali ini saya akan mempersentsikan hasil penelitian saya yang berjudul preferensi pengunjung terhadap ruang publik di kawasan tepi laut senggol parepare}
\end{frame}
}
...
\notec{Pada gambar kita lihat tepi laut senggol sebelum pengembangan. Gambar ini diambil pada tahun sebelum 2011.\scriptsize a2}
\notec{Gambar selanjutnya yaitu tepi laut setelah pengembangan atau kondisi saat ini. Pengembangan ini jelas meningkatkan kondisi tepi laut sekarang dengan penambahan naungan, lantai, dan tempat makan. \scriptsize a3}
...
{
\usebackgroundtemplate{%
\tikz\node[opacity=0.4,inner sep=0] {\includegraphics[height=\paperheight,width=\paperwidth]{bg1.png}};}
...
\end{frame}}
{\cca
\begin{frame}[label=tin]{Variabel penelitian: Elemen ruang}
...
\begin{frame}[label=tin]{Kerangka Konseptual}
\scalebox{0.6}{
\small
\centering
...
\notec{Kerangka konseptual yang digunakan adalah sebagai berikut \scriptsize b1}
\end{frame}
...
\item[] \item Perangkat lunak : (1) \textbf{R} untuk Crosstab dan (2) \textbf{Minitab} untuk Biplot
\end{itemize}}
\end{columns}
\notec{Penelitian ini menggunakan pendekatan kualitatif dan kuantitatif dengan penggunaan survei kuesioner.}
\notec{Data yang diambil adalah jawaban yang didapatkan dari responden pengunjugn tepi laut senggol melalui pertanyaan kuesioner.}
\notec{Teknik penyampelan adalah probalitas...}
\end{frame}}

\begin{frame}[label=obj]{Objek penelitian}
\centering
    \includegraphics[width=.72\textwidth]{figures/lokzi3.png}
    \notec{Objek penelitian ini terbagi atas dua bagian ruang yang disebut ruang A dan B. Luas ruang A meliputi daerah yang berwarna merah. Sedangkan ruang B meliputi area yang berwarna biru. Ruang A terbentang dari pelabuhan nusantara Parepare hingga batas ruang B. Sedangkan ruang B terbentang dari batas ruang A hingga pasar senggol di bagian utara.}
\end{frame}
...
\begin{minipage}{.45\textwidth}
    \includegraphics[width=\linewidth]{figures/mapsra.png}
\end{minipage}
\begin{minipage}{.45\textwidth}
    \includegraphics[width=\linewidth]{figures/mapsrb.png}
\end{minipage}
...
\notec{Selanjutnya potongan view dari ruang A, dan ruang B.}
\end{frame}}
...
\end{columns}
\notec{Pada penelitian ini mayoritas responden adalah berjenis kelamin lali-laki dengan jumlah 56 orang.

35 dari mereka tergolong kelompok usia dewasa. Ini hampir setengah dari keseluruhan responden.

Mayoritas responden merupakan orang bugis, ini dikarenakan kota parepare kebanyakan adalah bersuku bugis. Dan sisanya adlaah suku makassar ataupun toraja.
...
Elemen yang paling disukai pada ruang A adlah lebar jalan yang lebih dari 3 meter serta penchyaan tinggi. Sedangkan pada ruang B, elemen yang disukai adalah kebnyakan orientasi yang menghadap ke laut. }
...
\begin{frame}{Kesimpulan}\vspace{4pt}
Setelah melakukan analisis, sejumlah pertanyaan penelitian terjawab yang kemudian disimpulkan sebagai berikut:
\begin{itemize}
    \item<1-> Hasil dari studi ini mengindikasikan bahwa ruang A paling disukai dibandingkan ruang B di tepi laut Senggol. Ini menunjukan bahwa orang memberi perhatian pada elemen-elemen yang dimiliki ruang publik. Berbeda dengan penemuan Y. Zhang dimana orang condong pada tempat yang alami tanpa intervensi orang (lingkungan binaan).
...
\begin{frame}{Kesimpulan}\vspace{4pt}
\begin{itemize}
    \item Elemen yang paling disukai oleh pengunjung adalah lebar jalan (>3m) dalam memilih ruang tertentu. Pernyataan ini berkontribusi terhadap tingginya preferensi pada aspek aksesibilitas.
\end{itemize}
\end{frame}
...
{
\usebackgroundtemplate{%
\tikz\node[inner sep=0] {\includegraphics[height=\paperheight,width=\paperwidth]{bg3.png}};}
\setbeamercolor{normal text}{fg=white}
\usebeamercolor[fg]{normal text}
\begin{frame}<beamer>{}
