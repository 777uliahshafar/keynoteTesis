...
Berdasarkan tinjauan teori, beberapa penelitian menemukan preferensi terhadap fitur dan terkait taman ditengah kota.\newline

{\Large Hanya sedikit yang membahas tentang preferensi ruang yang ada di kawasan tepi laut.}
...
Kawasan revitalisasi tepi laut yang berhasil ditandai dengan keberhasilannya membawa pengunjung ke tepi laut, yang mana menciptakan keramaian pada suatu ruang. Keberhasilan ini dapat ditingkatkan berdasarkan kebutuhan pengguna, dengan mengetahui preferensi ruang dapat membantu mengelola pengembangan untuk memenuhi kebutuhan pengguna.

\notec{Pada tahun 2011, kota Parepare memulai perencanaan penataan kawasan tepi laut senggol. Penataan ini membangun ruang-ruang yang memiliki fitur atau atribut yang berbeda.
...
\begin{enumerate}
    ...
    \item Apa {\Large fitur ruang} yang paling penting terhadap pemilihan ruang? Apakah kepentingannya bervariasi diantara ruang-ruang?
...
\begin{frame}[label=tin]{Definisi} \vspace{4pt}
Fitur ruang adalah bagian dari \textit{setting} {\Large ruang publik }(seperti \textit{waterfront \& taman}) yang khas dan konteks lingkungan yang lebih luas.
...
\begin{frame}[label=tin]{Sub Variabel} \vspace{4pt}
...
\begin{tikzpicture}[node distance=2cm]
	\node (fit) [startstop, text width= 5cm] {\textbf{Fitur lingkungan}\\\small(aksesibilitas, fasilitas,estetika, keamanan, dan pemeliharaan)};

	\node (soc) [startstop, below of=fit, text width= 4cm, yshift=-.5cm, ] {\textbf{Karakter Orang}\\\small(umur,gender,ras dan status kegiatan ekonomi)};

	\node (pre) [startstop, right of=fit, text width= 4cm,yshift=-.5cm, xshift=4cm] {\textbf{Preferensi Ruang}\\ Ruang A atau Ruang B};
...
\notec{analisis data yang menggunakan statistik \textit{(crosstab)}, didapatkan hubungan antara variabel (fitur ruang) dan ruang yang terpilih.}
\end{frame}
...
\end{columns}
\notec{Fitur estetika dominan didukung oleh elemen alami yaitu kerapatan vegetasi dan kehadiran tumbuhan.}
...
\begin{frame}[label=obj]{Pemeliharaan} \vspace{4pt}
Ruang A\\
\begin{columns}[onlytextwidth]
\column{0.5\textwidth}
...
\begin{frame}[label=obj]{Pemeliharan} \vspace{4pt}
Ruang B\\
